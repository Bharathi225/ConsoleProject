...
public class StudentDatabase {
	...
	void toAdd(Connection connection) throws NullPointerException, SQLException{
		PreparedStatement insertStatement = connection.prepareStatement("INSERT INTO student_details values(?, ?, ?, ?)");
	...
	void toDisplayAll(Connection connection) throws NullPointerException, SQLException{
		...
		//student_details table is created in school database
		ResultSet result = stmt.executeQuery("SELECT * FROM student_details");
		while(result.next()) { 
	...
	void toDisplaybyId(Connection connection) throws NullPointerException, SQLException {
		PreparedStatement prepstmnt = connection.prepareStatement("SELECT * FROM student_details WHERE studentId = ? ");  
	...
	void toUpdate(Connection connection) throws NullPointerException, SQLException{
		...
		System.out.println("Enter the std to be updated: ");
		String std = scan.next();
		System.out.println("Enter the phnum to be updated: ");
		int contactNo = scan.nextInt();
		PreparedStatement updateStmt = connection.prepareStatement("UPDATE student_details SET studentName = ?,standard = ?,contactNo = ? WHERE studentId = ?");
		updateStmt.setString(1, name); //1st index name bcz 74th line 1st ? should be name
		updateStmt.setString(2, std);
		updateStmt.setLong(3, contactNo);
	...
	void toDelete(Connection connection) throws NullPointerException, SQLException{
		PreparedStatement pstmt = connection.prepareStatement("DELETE FROM student_details WHERE studentId = ? ");
